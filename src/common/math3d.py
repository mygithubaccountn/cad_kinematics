...
def bbox_aabb_gap(a: np.ndarray, b: np.ndarray) -> float:
    """Euclidean gap between two AABB (2,3) min/max. 0 if overlapping."""
    a = np.asarray(a, dtype=np.float64).reshape(2, 3)
    b = np.asarray(b, dtype=np.float64).reshape(2, 3)
    delta = 0.0
    for i in range(3):
        if a[1, i] < b[0, i]:
            d = float(b[0, i] - a[1, i])
